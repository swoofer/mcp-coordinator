...
describe("WorkingFilesTracker", () => {
  ...
  it("start inserts a row with claim_until ~30min in the future", () => {
    ...
    const row = getDb()
      .prepare("SELECT * FROM working_files WHERE agent_id=?")
      .get("agentA") as any;
...
describe("working-files-tracker org_id scoping", () => {
  ...
  it("start writes org_id into working_files", () => {
    ...
    const row = getDb()
      .prepare("SELECT org_id FROM working_files WHERE agent_id = ?")
      .get("agentA") as { org_id: string };
