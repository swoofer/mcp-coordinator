...
export function registerAgentTools(
  ...
  server.tool("register_agent", "Register agent as online with module list", {
    ...
    modules: z.array(z.string()).describe("Module IDs (as used in the dependency map) this agent owns/works on."),
  }, { readOnlyHint: false, destructiveHint: false, idempotentHint: true, title: "Register agent" }, async ({ agent_id, name, modules }, extra) => {
  ...
  server.tool("list_agents", "List registered agents", {
    online_only: z.boolean().optional().describe("If true, only return agents currently marked online."),
  }, { readOnlyHint: true, title: "List agents" }, async ({ online_only }, extra) => {
    const claims = getSessionClaims(extra.sessionId ?? "");
    if (!claims) throw new Error("Session has no captured claims (auth bug)");
  ...
  server.tool("heartbeat", "Update agent activity status and last seen timestamp", {
    ...
    current_file: z.string().optional().describe("Repo-relative path of the file the agent is currently working on, if any."),
    current_thread: z.string().optional().describe("Thread ID the agent is currently engaged in, if any."),
  }, { readOnlyHint: false, destructiveHint: false, idempotentHint: true, title: "Heartbeat" }, async ({ agent_id, current_file, current_thread }, extra) => {
    const claims = getSessionClaims(extra.sessionId ?? "");
    if (!claims) throw new Error("Session has no captured claims (auth bug)");
  ...
  server.tool("agent_activity", "Get activity status for all online agents", {}, { readOnlyHint: true, title: "Get agent activity" }, async (_args, extra) => {
    const claims = getSessionClaims(extra.sessionId ?? "");
    if (!claims) throw new Error("Session has no captured claims (auth bug)");
