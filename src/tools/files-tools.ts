...
export function registerFilesTools(
  ...
  }, { readOnlyHint: true, title: "List hot files" }, async ({ since_minutes }, extra) => {
  ...
  }, { readOnlyHint: true, title: "Get session files" }, async ({ session_id }, extra) => {
    const claims = getSessionClaims(extra.sessionId ?? "");
    if (!claims) throw new Error("Session has no captured claims (auth bug)");
  ...
  server.tool("check_file_conflict", "Check if another agent is editing a file", {
    ...
    within_minutes: z.number().optional().describe("Look-back window in minutes. Defaults to 30."),
  }, { readOnlyHint: true, title: "Check file conflict" }, async ({ file_path, agent_id, within_minutes }, extra) => {
    const claims = getSessionClaims(extra.sessionId ?? "");
    if (!claims) throw new Error("Session has no captured claims (auth bug)");
