...
export class FileTracker {
  ...
  getBySession(sessionId: string): FileActivity[] {
    const db = getDb();
    return db.prepare("SELECT * FROM file_activity WHERE session_id = ? ORDER BY created_at").all(sessionId) as FileActivity[];
  }

  getHotFiles(sinceMinutes: number = 30): { file_path: string; agent_count: number; agents: string[] }[] {
    const db = getDb();
    ...
    ).all(sinceMinutes) as { file_path: string; agent_count: number; agents: string }[];
    return rows.map((r) => ({
  ...
  checkFileConflict(filePath: string, agentId: string, withinMinutes: number = 30): { conflict: boolean; agents: string[] } {
    const db = getDb();
    const rows = db.prepare(
      `SELECT DISTINCT agent_id FROM file_activity
       WHERE file_path = ? AND agent_id != ?
       AND created_at > datetime('now', '-' || ? || ' minutes')`
    ).all(filePath, agentId, withinMinutes) as { agent_id: string }[];
