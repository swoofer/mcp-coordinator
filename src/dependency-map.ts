...
export class DependencyMapper {
  ...
  setDependencies(
    orgId: string,
    moduleId: string,
    params: { depends_on: string[]; exports: string[]; owners: string[] },
  ): void {
    const db = getDb();
    db.prepare(
      ...
      JSON.stringify(params.exports),
      JSON.stringify(params.owners),
    );
  }
  ...
  getDependencies(
    orgId: string,
    moduleId: string,
  ): { depends_on: string[]; exports: string[]; owners: string[] } | null {
    const db = getDb();
    const row = db
      .prepare(
    ...
    if (!row) return null;
    return {
      depends_on: safeJsonParse<string[]>(
        row.depends_on,
        [],
        undefined,
        "dependency-map.getDependencies:depends_on",
      ),
      exports: safeJsonParse<string[]>(
        row.exports,
        [],
        undefined,
        "dependency-map.getDependencies:exports",
      ),
      owners: safeJsonParse<string[]>(
        row.owners,
        [],
        undefined,
        "dependency-map.getDependencies:owners",
      ),
    };
  }
  ...
  listOwners(orgId: string): string[] {
    const db = getDb();
    const rows = db.prepare("SELECT owners FROM dependency_map WHERE org_id = ?").all(orgId) as {
      owners: string;
    }[];
    const all = new Set<string>();
    for (const r of rows) {
      safeJsonParse<string[]>(r.owners, [], undefined, "dependency-map.listOwners:owners").forEach(
        (o) => all.add(o),
      );
    }
    ...
    for (const row of rows) {
      map[row.module_id] = {
        ...
        depends_on: safeJsonParse<string[]>(
          ...
          [],
          undefined,
          "dependency-map.getMap:depends_on",
        ),
        exports: safeJsonParse<string[]>(
          row.exports,
          [],
          undefined,
          "dependency-map.getMap:exports",
        ),
    ...
    withTransaction(db, () => {
      for (const [id, info] of Object.entries(map)) {
        stmt.run(
          orgId,
          id,
          JSON.stringify(info.depends_on),
          JSON.stringify(info.exports),
          JSON.stringify(info.owners),
        );
      }
    });
  }
  ...
  getModuleInfo(orgId: string, moduleId: string): ModuleInfo | null {
    const db = getDb();
    const row = db
      .prepare("SELECT * FROM dependency_map WHERE org_id = ? AND module_id = ?")
      .get(orgId, moduleId) as
      | {
          module_id: string;
          depends_on: string;
          exports: string;
    ...
    return {
      ...
      depends_on: safeJsonParse<string[]>(
        ...
        [],
        undefined,
        "dependency-map.getModuleInfo:depends_on",
      ),
      exports: safeJsonParse<string[]>(
        row.exports,
        [],
        undefined,
        "dependency-map.getModuleInfo:exports",
      ),
      owners: safeJsonParse<string[]>(
        row.owners,
        [],
        undefined,
        "dependency-map.getModuleInfo:owners",
      ),
    };
  }
