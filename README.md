...
## Capabilities at a glance (v0.10.6)
...
| **Admin UI for org / user management** | — | ✅ Shipped v0.10.6 — browser console at `/dashboard/admin.html` (admin role required); manage organizations, allowlists, and per-user roles; CSRF double-submit + per-IP rate limit (30/min) on mutations; last-admin demote protection; audit trail under `admin.org.*` + `admin.user.role_changed` |
...
- **npm** — Requires Node.js 20+. Three usage styles:
  - `npx mcp-coordinator <cmd>` (zero install, downloads to npm cache)
  - `npm install -g mcp-coordinator` then `mcp-coordinator <cmd>` (recommended for daemons)
  - `npm install mcp-coordinator` in a project, then `npx mcp-coordinator <cmd>` from project root (pinned per-project)
...
All command examples below use the bare `mcp-coordinator <cmd>` form; prefix with `npx ` if you use a non-global install.
...
mcp-coordinator server start --daemon
#                 ── or ──
# npx mcp-coordinator server start --daemon
...
Per-version detail for v0.5.0 → v0.10.6 lives below. The [Capabilities at a glance](#capabilities-at-a-glance-v0106) matrix at the top of this README is the current-state summary.
